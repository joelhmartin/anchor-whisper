...
test("has_speech ignores whisper's non-speech glyphs", function()
  eq(core.has_speech("…"), false)
  eq(core.has_speech("♪♪"), false)
  eq(core.has_speech(" — "), false)
  eq(core.has_speech("’"), false)
  eq(core.has_speech("… okay"), true)
end)
...
test("join_at_cursor adds no space before attaching punctuation or after a path/hyphen", function()
  eq(core.join_at_cursor({ before = "I went to the store", after = "" }, ", and then left."), ", and then left.")
  eq(core.join_at_cursor({ before = "done", after = "" }, "."), ".")
  eq(core.join_at_cursor({ before = "/Users/bif/", after = "" }, "developer"), "developer")
  eq(core.join_at_cursor({ before = "https://example.com/", after = "" }, "docs"), "docs")
  eq(core.join_at_cursor({ before = "re-", after = "" }, "entry"), "entry")
  eq(core.join_at_cursor({ before = "snake_", after = "" }, "case"), "case")
  eq(core.join_at_cursor({ before = "mail@", after = "" }, "example"), "example")
end)

test("join_at_cursor trims stray spaces the model added", function()
  eq(core.join_at_cursor({ before = "dinner ", after = "" }, " and then left"), "and then left")
  eq(core.join_at_cursor({ before = "dinner", after = "" }, " and then left "), " and then left")
  eq(core.join_at_cursor({ before = "Notes:\n", after = "" }, "\n- item"), "\n- item")
end)
...
test("join_at_cursor adds a trailing space when text follows the cursor immediately", function()
  ...
  eq(core.join_at_cursor({ before = "", after = "(see below)" }, "Start."), "Start. ")
  eq(core.join_at_cursor({ before = "", after = "- buy milk" }, "First"), "First ")
  eq(core.join_at_cursor({ before = "a", after = "b" }, ""), "")
end)

test("build_user_message strips the block delimiters from document text", function()
  local m = core.build_user_message("x", { before = "a >>> b <<< c", after = ">>>" })
  local _, n = m:gsub(">>>", "")
  eq(n, 3, "exactly the three closing fences")
  assert(m:find("a  b  c", 1, true), "fences removed from context")
end)

test("API adapters keep newlines in the model output", function()
  eq(core.backends.openai.parse('{"choices":[{"message":{"content":"- a\\n- b\\n"}}]}'), "- a\n- b")
  eq(core.backends.anthropic.parse('{"content":[{"type":"text","text":"  One.\\n\\nTwo.  "}],"stop_reason":"end_turn"}'), "One.\n\nTwo.")
  eq(core.backends.gemini.parse('{"candidates":[{"content":{"parts":[{"text":"x\\r\\ny"}]},"finishReason":"STOP"}]}'), "x\ny")
end)
